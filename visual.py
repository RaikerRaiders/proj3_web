...
roads_names = ['Биомедицина', 'Нанотехнологии, композиты, пр.', 'Аэрокосмос', 'Сельское хозяйство',
               'Информационные технологии', 'Другое', 'Вовлечение молодeжи в работу средств массовой информации',
               'Работа с молодeжью, находящейся в социально-опасном положении', 'Творческая деятельность',
               'Формирование у молодeжи семейных ценностей', 'Волонтeрская деятельность',
               'Здоровый образ жизни и занятия спортом, культура безопасности']
...
def clusters_show_2d(x_col, y_col, clusters_count=4, scale=0.9):
    ...
    for i in range(clusters_count):
        ...
        dfs.append(pd.DataFrame(
            {'name': 'Кластер ' + str(i + 1), 'color': [markers[i] for j in range(len(x))], 'x': x, 'y': y}))
...
app.layout = html.Div([
    ...
    html.Div(
        [
            html.H2('ABY', className='display-3', style={'color': 'white'}),
            ...
            dbc.Nav(
                [
                    ...
                    dbc.NavLink("Контактные данные", disabled=True, style={
                        "position": "fixed",
                        "bottom": "60px"
                    }),
                    dbc.NavLink("+7 (928) 225-00-10", disabled=True, style={
                        "position": "fixed",
                        "bottom": "40px"
                    }),
                    dbc.NavLink("raikerraiders@gmail.com", disabled=True, style={
                        "position": "fixed",
                        "bottom": "20px"
                    })
                ],
            ...
            html.Div([
                dcc.Dropdown(
                    ...
                    options=[
                        ...
                        {'label': 'Кол-во добровольцев по направлениям молодежных политик',
                         'value': 'volontiers_amount_reg'},
                        {'label': 'Уникальные пользователи интернет-ресурсов молодежных направлений',
                         'value': 'uniq_users'},
                        {'label': 'Бюджет, выделенный на маркетинг молодежных политик', 'value': 'budget_marketing'},
                        {'label': 'Количество маркетинговых единиц (новостей, статей, постов и тд.)',
                         'value': 'marketing_units'},
                        {'label': 'Численность региона', 'value': 'population'},
                    ],
                    multi=True, value=['volontiers_amount_reg', 'budget2youth_reg'], className='dropdown',
...
def update_output(value):
    ...
    charac_div = html.Div([
        html.P(children='Краткая характеристика региона: ' + data[0], className='header-description',
               style={'margin-bottom': '8px'}),
        html.P(
            children='Количество средств на человека в рамках проведения мероприятий по направлениям молодежной политики: {:.2f} руб./участника'.format(
                data[5] / data[6]), className='header-description', style={'margin-bottom': '8px'}),
        html.P(
            children='Отношение бюджета, выделяемого на направления молодежной политики, к общему бюджету региона: {:.6f}'.format(
                data[5] / data[4]), className='header-description', style={'margin-bottom': '8px'}),
        html.P(
            children='Отношение количества граждан, вовлеченных в добровольческую деятельность, к общему населению региона: {:.4f}'.format(
                data[12] / data[11]), className='header-description', style={'margin-bottom': '8px'}),
        html.P(children='Процентное соотношение добровольцев из школ/СПО/ВУЗов: ', className='header-description',
               style={'margin-bottom': '8px'}),
